...
from django.db.models import (
    Case, Count, DecimalField, ExpressionWrapper, F, IntegerField, Q, Sum, Value, When,
)
...
REPORTS = {
    ...
    "top_products": {
        ...
        "search": False,
        "filters": lambda: [
            _f_daterange(), _f_store(), _f_category(),
...
class ReportBuilderService:
    ...
    def export_rows(params) -> tuple[str, list, list, list]:
        ...
        columns, rows_or_qs, row_fn, summary = ReportBuilderService._run(params)
        if row_fn is None:
            rows = list(rows_or_qs)[:EXPORT_MAX_ROWS]
        else:
            rows = [row_fn(obj) for obj in rows_or_qs[:EXPORT_MAX_ROWS]]
