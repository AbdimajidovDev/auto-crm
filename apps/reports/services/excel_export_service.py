...
class _ReportWorkbook:
    ...
    def info_block(self, ws, start_row: int, rows: list[tuple[str, object, bool]]) -> int:
        """
        ...
        f_int = self._fmt(bold=True, bg_color="#FFFFFF", border=1, border_color=GRID,
                          align="right", num_format="#,##0")
        for i, (label, value, is_money) in enumerate(rows):
            r = start_row + i
            ws.write_string(r, 0, label, f_label)
            ws.write_number(r, 1, _num(value), f_money if is_money else f_int)
...
class ExcelExportService:
    ...
    # Bo'limlab eksport uchun ruxsat etilgan kalitlar (?section=...)
    SECTIONS = (
        "branches", "categories", "top_products", "payments",
        "cards", "expenses", "customer_debts", "supplier_debts",
        "low_stock", "recent_sales",
    )

    @staticmethod
    def generate_report(data: dict, meta: dict | None = None, sections: set[str] | None = None):
        """
        sections=None — to'liq hisobot (barcha varaqlar + Umumiy dashboard).
        sections={'top_products'} kabi — faqat so'ralgan bo'lim varaq(lar)i
        (Umumiy varaq boshqa varaqlardagi jadvallarga diagramma orqali
        bog'langani uchun bo'lim rejimida yaratilmaydi).
        """
        output = io.BytesIO()
        ...
        def want(key: str) -> bool:
            return full or key in sections

        # Varaqlar tartibi (yaratilish tartibida ko'rinadi)
        ws_main  = b.sheet("Umumiy", TITLE_BG) if full else None
        ws_br    = b.sheet("Filiallar",           SERIES[0]) if want("branches") else None
        ws_cat   = b.sheet("Kategoriyalar",       SERIES[2]) if want("categories") else None
        ws_top   = b.sheet("Top mahsulotlar",     SERIES[6]) if want("top_products") else None
        ws_pay   = b.sheet("To'lovlar",           SERIES[3]) if want("payments") else None
        ws_card  = b.sheet("Kartalar kesimi",     SERIES[4]) if want("cards") else None
        ws_exp   = b.sheet("Xarajatlar",          SERIES[1]) if want("expenses") else None
        ws_cdebt = b.sheet("Mijoz qarzlari",      SERIES[7]) if want("customer_debts") else None
        ws_sdebt = b.sheet("Taminotchi qarzlari", BAD_RED) if want("supplier_debts") else None
        # Bosh sahifadagi bloklarga mos qo'shimcha varaqlar
        ws_low    = b.sheet("Kam qoldiq",       SERIES[5]) if want("low_stock") else None
        ws_recent = b.sheet("Oxirgi sotuvlar",  SERIES[1]) if want("recent_sales") else None

        # Umumiy varaq diagrammalari uchun manbalar (bloklar o'tkazilsa bo'sh qoladi)
        branches: list = []
        cat_pairs: list = []
        pay_pairs: list = []
        ...
        if ws_sdebt is not None:
            sdebts = (data.get("debts") or {}).get("supplierDebts") or []
            b.title_band(ws_sdebt, "Ta'minotchi qarzlari", 1)
            # Yangi supplierStatistics bloki (bo'lsa) — jadval tepasida mini panel.
            # Eski backend'da blok yo'q — panel shunchaki chiqmaydi.
            sstats = data.get("supplierStatistics") or {}
            table_start = 3
            if sstats:
                table_start = b.info_block(ws_sdebt, 3, [
                    ("Ta'minotchilar soni",   sstats.get("supplierCount"),        False),
                    ("Mahsulot turlari",      sstats.get("distinctProductCount"), False),
                    ("Davrdagi xarid summasi", sstats.get("totalPurchaseAmount"), True),
                    ("Jami qarz",             sstats.get("totalDebt"),            True),
                ])
            cols = [
            ...
            _, fd, ld = b.table(ws_sdebt, table_start, cols, rows)
            if rows:
                b.data_bars(ws_sdebt, fd, ld, 1, BAD_RED)
            else:
                b.empty_note(ws_sdebt, table_start + 1)

        # ======================= 10. KAM QOLDIQ =======================
        if ws_low is not None:
            low_rows = data.get("lowStock") or []
            b.title_band(ws_low, "Kam qolgan mahsulotlar", 4)
            cols = [
                {"header": "Mahsulot",    "width": 42, "kind": "text"},
                {"header": "SKU",         "width": 14, "kind": "text"},
                {"header": "Kategoriya",  "width": 20, "kind": "text"},
                {"header": "Qoldiq",      "width": 10, "kind": "int"},
                {"header": "Min. qoldiq", "width": 12, "kind": "int"},
            ]
            rows = [[r.get("name"), r.get("sku"), r.get("category"),
                     r.get("stock"), r.get("min_stock")] for r in low_rows]
            _, fd, ld = b.table(ws_low, 3, cols, rows, totals=False)
            if not rows:
                b.empty_note(ws_low, 4)

        # ======================= 11. OXIRGI SOTUVLAR =======================
        if ws_recent is not None:
            recent = data.get("recentSales") or []
            b.title_band(ws_recent, "Oxirgi sotuvlar", 5)
            cols = [
                {"header": "Chek №",     "width": 10, "kind": "int"},
                {"header": "Sana",       "width": 17, "kind": "text"},
                {"header": "Do'kon",     "width": 22, "kind": "text"},
                {"header": "Mijoz",      "width": 26, "kind": "text"},
                {"header": "Summa",      "width": 15, "kind": "money", "total": "sum"},
                {"header": "To'lov",     "width": 13, "kind": "text"},
            ]
            rows = [[r.get("id"), r.get("date"), r.get("store"), r.get("customer"),
                     r.get("total"), r.get("payment")] for r in recent]
            _, fd, ld = b.table(ws_recent, 3, cols, rows)
            if not rows:
                b.empty_note(ws_recent, 4)
        ...
        # Bo'lim rejimida birinchi (yagona) varaq ochiq turadi
        if ws_main is not None:
            ws_main.activate()
